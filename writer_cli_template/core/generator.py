...
class ContentGenerator:
    ...
    def _load_manifest(self) -> Dict[str, Any]:
        """加载作品元数据"""
        manifest_path = get_project_root() / "MANIFEST.md"
        ...
        result = {}
        # 提取书本名称
        match = re.search(r"## 1\. 书本名称\s*\n```\s*\n(.+?)\s*\n```", content, re.DOTALL)
        if match:
            result["book_name"] = match.group(1).strip()
        # 提取主角名
        match = re.search(r"\*\*主角1.*?\*\*\s*\n```\s*\n(.+?)\s*\n```", content, re.DOTALL)
        if match:
            result["main_character"] = match.group(1).strip()
        match = re.search(r"\*\*主角2.*?\*\*\s*\n```\s*\n(.+?)\s*\n```", content, re.DOTALL)
        if match:
            result["secondary_character"] = match.group(1).strip()
        # 提取简介
        match = re.search(r"## 4\. 作品简介\s*\n```\s*\n(.+?)\s*\n```", content, re.DOTALL)
        if match:
            result["synopsis"] = match.group(1).strip()[:200]
        # 提取标签
        tags = re.findall(r"- \[x\] (.+)", content)
        if tags:
    ...
    def _format_manifest(self) -> str:
        ...
        if not self.manifest:
            return "（作品元数据未填写，请从上下文推断）"
        parts = []
        if "book_name" in self.manifest:
            parts.append(f"书名：《{self.manifest['book_name']}》")
        if "tags" in self.manifest:
            parts.append(f"标签：{self.manifest['tags']}")
        if "main_character" in self.manifest:
            parts.append(f"主角：{self.manifest['main_character']}")
        if "secondary_character" in self.manifest:
            parts.append(f"女主/副主角：{self.manifest['secondary_character']}")
        if "synopsis" in self.manifest:
            parts.append(f"简介：{self.manifest['synopsis']}")
        return "\n".join(parts)
    ...
    def generate_chapter(
        ...
        # 构建 Prompt
        manifest_str = self._format_manifest()
